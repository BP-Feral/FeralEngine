...
def get_build_version(short):
    ...
    if not short:
        if os.getenv("FERAL_VERSION_STATUS") is not None:
            status = str(os.getenv("FERAL_VERSION_STATUS"))
        v += ".%s.%s" % (status, name)
    return v
...
def generate_bundle_apple_embedded(platform, framework_dir, framework_dir_sim, use_mkv, target, source, env):
    ...
    # Template bundle.
    app_prefix = "feral." + platform
    rel_prefix = "libferal." + platform + "." + "template_release"
    dbg_prefix = "libferal." + platform + "." + "template_debug"
    if env.dev_build:
        app_prefix += ".dev"
        rel_prefix += ".dev"
        dbg_prefix += ".dev"
    ...
    if rel_target_bin != "":
        ...
        shutil.copy(
            rel_target_bin, app_dir + "/libferal." + platform + ".release.xcframework/" + framework_dir + "/libferal.a"
        )
    if dbg_target_bin != "":
        print(f' Copying "{platform}" debug framework')
        shutil.copy(
            dbg_target_bin, app_dir + "/libferal." + platform + ".debug.xcframework/" + framework_dir + "/libferal.a"
        )
    if rel_target_bin_sim != "":
        print(f' Copying "{platform}" (simulator) release framework')
        shutil.copy(
            rel_target_bin_sim,
            app_dir + "/libferal." + platform + ".release.xcframework/" + framework_dir_sim + "/libferal.a",
        )
    if dbg_target_bin_sim != "":
        print(f' Copying "{platform}" (simulator) debug framework')
        shutil.copy(
            dbg_target_bin_sim,
            app_dir + "/libferal." + platform + ".debug.xcframework/" + framework_dir_sim + "/libferal.a",
        )

    # Remove other platform xcframeworks
    for entry in os.listdir(app_dir):
        if entry.startswith("libferal.") and entry.endswith(".xcframework"):
            parts = entry.split(".")
