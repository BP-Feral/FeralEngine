...
def version_info_builder(target, source, env):
    with methods.generated_wrapper(str(target[0])) as file:
        file.write(
            """\
#define FERAL_VERSION_SHORT_NAME "{short_name}"
#define FERAL_VERSION_NAME "{name}"
#define FERAL_VERSION_MAJOR {major}
#define FERAL_VERSION_MINOR {minor}
#define FERAL_VERSION_PATCH {patch}
#define FERAL_VERSION_STATUS "{status}"
#define FERAL_VERSION_BUILD "{build}"
#define FERAL_VERSION_MODULE_CONFIG "{module_config}"
#define FERAL_VERSION_WEBSITE "{website}"
#define FERAL_VERSION_DOCS_BRANCH "{docs_branch}"
#define FERAL_VERSION_DOCS_URL "https://docs.godotengine.org/en/" FERAL_VERSION_DOCS_BRANCH
...
const char *const FERAL_VERSION_HASH = "{git_hash}";
const uint64_t FERAL_VERSION_TIMESTAMP = {git_timestamp};
""".format(**source[0].read())
        )
...
def make_license_header(target, source, env):
    ...
    with methods.generated_wrapper(str(target[0])) as file:
        file.write(f"""\
inline constexpr const char *FERAL_LICENSE_TEXT = {{
